...
public interface IConductorDao extends PagingAndSortingRepository<Conductor, String> {
	...
	@Query("select p from Conductor p inner join Adscripcion a on p.adscripcion.id_adscripcion = a.id_adscripcion where a.id_adscripcion = ?1 and fecha_baja is null")
	...
	@Query("select p from Conductor p inner join Adscripcion a on p.adscripcion.id_adscripcion = a.id_adscripcion where a.id_adscripcion= ?1 and p.fecha_baja is null and (p.no_empleado like %?2% or p.nombre like %?2% or p.apellido1 like %?2% or p.apellido2 like %?2% or p.adscripcion.nombre_adscripcion like %?2%)")
	public Page<Conductor> findCondElemnAreaPage(Long id_adscripcion,String elemento,Pageable pageable);
	
	@Query("select p from Conductor p inner join Adscripcion a on p.adscripcion.id_adscripcion = a.id_adscripcion where a.id_adscripcion= ?1 and p.fecha_baja is not null and (p.no_empleado like %?2% or p.nombre like %?2% or p.apellido1 like %?2% or p.apellido2 like %?2% or p.adscripcion.nombre_adscripcion like %?2%)")
