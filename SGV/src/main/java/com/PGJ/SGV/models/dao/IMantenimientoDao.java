...
public interface IMantenimientoDao extends PagingAndSortingRepository<Mantenimiento, Long>{
	...
	@Query("select m from Mantenimiento m inner join Vehiculo v on m.vehiculo.id_vehiculo = v.id_vehiculo inner join Adscripcion a on v.adscripcion.id_adscripcion=a.id_adscripcion where a.id_adscripcion like ?1")
	...
	@Query("select m from Mantenimiento m inner join Vehiculo v on m.vehiculo.id_vehiculo = v.id_vehiculo inner join Adscripcion a on v.adscripcion.id_adscripcion=a.id_adscripcion where v.id_vehiculo = ?1 and a.id_adscripcion like ?2")
