...
public interface IVehiculoPlacasDao extends PagingAndSortingRepository<VehiculoPlacas, Long>{
	...
	@Query("select v from VehiculoPlacas v inner join Vehiculo l on v.vehiculo.id_vehiculo = l.id_vehiculo where v.fecha_inicio = (select max(p.fecha_inicio) from VehiculoPlacas p) and l.id_vehiculo = ?1")
